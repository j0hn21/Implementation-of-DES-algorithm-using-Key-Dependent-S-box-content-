...
def F_function(Kn,Rn):
    ...
    for j in range(0,40,8):
        res[0:8] = XOR(res[0:8],Kn[j:j+8])
    temp = [1,1,1,1,1,1,1,1]
    rescom = XOR(res,temp)
    resdec = bit_array_to_dec(res)
    rescomdec = bit_array_to_dec(rescom)
    F_value = []
    ...
    for i in range(0,8):
        ...
        #print("Original Sbox ",i," content: ",S_box_value)
        resdec = resdec * S_box_value + rescomdec
        resdec = resdec % 16 
        #print("Modified Sbox ",i," content: ",resdec)
        rescom = dec_to_bit_array(resdec)
        F_value.append(rescom)
